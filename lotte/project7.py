...
mc = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet14.h5',save_best_only=True, verbose=1)
...
model.compile(loss='categorical_crossentropy', optimizer=Adam(1e-4),
                metrics=['acc'])
# learning_history = model.fit_generator(train_generator,epochs=80, steps_per_epoch= len(x_train) / 64,
#     validation_data=valid_generator, callbacks=[early_stopping,lr,mc])

# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet14.h5')
result = model.predict(x_pred,verbose=True)

sub1 = pd.read_csv('../../data/image/sample.csv')
sub1['prediction'] = np.argmax(result,axis = 1)
sub1.to_csv('../../data/image/answer14_2.csv',index=False)
...
sub.to_csv('../../data/image/answer14.csv',index=False)
