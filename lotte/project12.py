...
#data load
x = np.load("C:/data/npy/P_project_x8.npy",allow_pickle=True)
y = np.load("C:/data/npy/P_project_y8.npy",allow_pickle=True)
x_pred = np.load('C:/data/npy/test3.npy',allow_pickle=True)


print(x_pred.shape)
print(x.shape)  # 65000,224,224
...
# from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
# cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet20.h5',monitor='val_acc',save_best_only=True, verbose=1)
# early_stopping = EarlyStopping(monitor='val_acc',patience= 8,verbose=1)
# lr = ReduceLROnPlateau(monitor='val_loss',patience= 4, factor=0.2,verbose=1)
...
# # predict
# model.load_weights('../../data/modelcheckpoint/lotte_projcet20.h5')
# result = model.predict(x_pred,verbose=True)

# sub = pd.read_csv('../../data/image/sample.csv')
# sub['prediction'] = np.argmax(result,axis = 1)
# sub.to_csv('../../data/image/answer20.csv',index=False)

# # #project 10부터는 쭉 이 모델 사용해보자 82점 /