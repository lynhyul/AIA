...
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet19.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet19.h5')
result = model.predict(x_pred,verbose=True) # 72000,1000

np.save('../../data/image/npy/x_pred6.npy', arr = result)
...
#         if percent > 0.01 and percent < 0.5:
...
# sub.to_csv('../../data/image/answer19_1.csv',index=False)

# # #project 10부터는 쭉 이 모델 사용해보자 81.5점 / 18,19