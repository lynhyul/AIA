...
#data load
x = np.load("C:/data/npy/P_project_x11.npy",allow_pickle=True)  # 256
y = np.load("C:/data/npy/P_project_y11.npy",allow_pickle=True)
x_pred = np.load('C:/data/npy/test256.npy',allow_pickle=True)
...
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet24.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet24.h5')
result = model.predict(x_pred,verbose=True)

sub = pd.read_csv('../../data/image/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('../../data/image/answer24.csv',index=False)
