...
#data load
x = np.load("C:/data/npy/P_project_x9.npy",allow_pickle=True)
y = np.load("C:/data/npy/P_project_y9.npy",allow_pickle=True)
...
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet22.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet22.h5')
result = model.predict(x_pred,verbose=True)

sub = pd.read_csv('../../data/image/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('../../data/image/answer22.csv',index=False)
