...
#data load
x = np.load("C:/data/npy/P_project_x7.npy",allow_pickle=True)
y = np.load("C:/data/npy/P_project_y7.npy",allow_pickle=True)
x_pred = np.load('C:/data/npy/test3.npy',allow_pickle=True)

print(x_pred.shape)
print(x.shape)  # 58000,224,224,3
...
# from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
# cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet17.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet17.h5')
result = model.predict(x_pred,verbose=True)

np.save('../../data/image/npy/x_pred7.npy', arr = result)

# sub = pd.read_csv('../../data/image/sample.csv')
# sub['prediction'] = np.argmax(result,axis = 1)
# sub.to_csv('../../data/image/answer17.csv',index=False)
