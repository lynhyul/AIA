...
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau
cp = ModelCheckpoint('../../data/modelcheckpoint/lotte_projcet27.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../../data/modelcheckpoint/lotte_projcet27.h5')
result = model.predict(x_pred,verbose=True) # 72000,1000

np.save('../../data/image/npy/x_pred4.npy', arr = result)


# sub = pd.read_csv('../../data/image/sample.csv')
# sub['prediction'] = np.argmax(result,axis = 1)
# sub.to_csv('../../data/image/answer27.csv',index=False)
