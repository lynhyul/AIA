...
# submission 데이터 불러오기
sub = pd.read_csv('../data/csv/sample_submission.csv')
...
for q in quantiles:
    ...
    history = model.fit(w1.train, validation_data=w1.val, epochs=20, callbacks=[es,lr])
...
sub.to_csv('../data/csv/submission_dense1_nn.csv',index=False)
### AutoRegressive LSTM
...
sub.to_csv('../data/csv/submission_lstm1_nn.csv',index=False)