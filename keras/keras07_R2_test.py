...
i = 0
for i in range(1,6) :
    model.add(Dense(100-i*18))
model.add(Dense(1))

#3. 컴파일, 훈련
model.compile(loss='mse', optimizer = 'adam', metrics = 'mae')
model.fit(x_train, y_train, epochs = 100, batch_size=1, validation_split= 0.8)   #x_train , y_train을 validation_split =0.2로 20%를 나누어쓰겠다.
...
print("RSE : ", RMSE (y_test, y_predict))
