...
imgs_dir = np.array(sorted(glob.glob('../data/csv/Dacon3/dirty_mnist_2nd/*')))[:]
labels = np.array(labels_df.values[:,1:])

test_imgs_dir = np.array(sorted(glob.glob('../data/csv/Dacon3/test_dirty_mnist_2nd/*')))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('../data/modelcheckpoint/0227_EfficientNetB7-fold{}.pt'.format(fold)))
        model.eval()
